...
격리 Restore, table row와 sequence 값·FK orphan·vector 정합성, aggregate
object·constraint count parity, 대표 관계 query와 임시 환경 cleanup까지 한 번의
수동 복구 흐름으로 검증하는 데 초점을 맞췄다.
...
- 14개 table row count, aggregate object·constraint count, sequence 값, FK orphan,
  pgvector와 대표 관계 query를 Production baseline과 비교했다.
...
- 정합성
  ...
  - Table 14, sequence 14, index 45, constraint 83: aggregate count parity 확인
  - Constraint 유형별 PK 14, FK 11, unique 11, check 47: count parity 확인
...
- PostgreSQL 17.6·pgvector 0.8.0 격리 Restore와 aggregate object count 검증이
  통과했다.
- Production/Restore row count와 sequence 값이 일치했고, object·constraint는
  aggregate 및 유형별 count parity를 확인했다. FK orphan, vector와 대표 query
  검증도 통과했다.
...
45개 index와 83개 constraint의 aggregate count parity, 14개 sequence 값,
FK orphan 0건, 1536차원 vector와 대표 관계 query를 비교해 복구 정합성을
확인했다. Credential과 데이터 본문은 evidence에서 제외하고, 임시 환경 cleanup
및 검증 artifact 보존 정책까지 Runbook으로 남겼다.
