...
- provider response parsing이 JSON object와 정상 field type을 가정해,
  non-object JSON이나 `NaN`/`inf` confidence가 제어되지 않은 오류 또는 잘못된
  summary output으로 이어질 수 있었다.
...
- Approved Fix 2를 적용해 provider response의 object/field/list item 타입과
  confidence 유효 범위를 검증하고, 잘못된 payload를 `ValueError`로 거부하도록
  수정했다.
...
- provider response는 structured JSON으로 parsing하고 JSON/markdown report에만
  남긴다.
- parsing 결과가 JSON object인지 확인하고 `title_ko`, `summary_ko`,
  `key_points`, `keywords`, `confidence`의 타입을 검증한다.
- `confidence`는 finite number이면서 `0~1` 범위인 경우에만 허용한다.
- provider payload validation 실패는 제어된 `ValueError`로 처리한다.
...
### Provider payload를 유연하게 coercion

잘못된 text/list field를 문자열 또는 list로 자동 변환하거나 confidence를 범위
안으로 보정할 수 있지만, 외부 provider 응답 오류를 숨기고 품질 문제를 정상
결과처럼 기록할 수 있어 제외했다.
...
- raw text 전문을 public output에서 제거해 used article 추적 가능성과 데이터
  최소화를 함께 확보했다.
- provider 응답 경계에서는 엄격한 validation과 명시적 실패를 선택해 malformed
  output이 report에 정상 summary로 포함되는 것을 방지했다.
...
- 엄격한 provider payload validation은 일부 복구 가능한 응답도 실패 처리할 수
  있지만, report MVP 단계에서는 오류를 숨기지 않는 편이 더 안전하다.
...
- Latest focused unittest after Approved Fix 2: 20 tests passed.
- Latest full unittest after Approved Fix 2: 95 tests passed.
- CLI help: passed.
- Approved Fix 2 변경 파일 대상 `git diff --check`: passed.
- 전체 `git diff --check`: 기존의 unrelated CodeRabbit review 파일 trailing
  whitespace 때문에 실패했다. 해당 review 파일은 수정하지 않았다.
...
Approved Fix 2 테스트에서 다음을 확인했다.

- JSON object가 아닌 provider response 거부
- text/list field와 list item의 잘못된 타입 거부
- `NaN`/`inf` 등 non-finite confidence 거부
- `0~1` 범위를 벗어난 confidence 거부
...
- 두 provider 결과는 markdown report에만 기록되었으며 topic summary DB
  write는 수행하지 않았다.
- Approved Fix 2 적용 후 Codex deterministic report 재생성은 현재 process에
  `DATABASE_URL`이 없어 DB 접근 전에 실패했다. 기존 human-approved actual-data
  report는 덮어쓰지 않았다.
- API, K8s, CronJob, deployment, rollout은 수행하지 않았다.
- production deployment/verification과 PR merge는 pending이다.
...
- deterministic/provider 검증 모두 DB write와 raw extraction 없이 report만
  생성했다.
- malformed provider payload가 제어된 validation error로 거부되는 것을 unit
  test로 확인했다.
- Approved Fix 2 검증 중 raw extraction, DB write, provider call은 수행하지
  않았다.
...
품질 차이, 외부 응답 validation, 데이터 노출 경계, 운영 안전성을 검토할 수
있는 기반을 마련했다.
...
parsing 및 엄격한 payload validation을 추가했다. `gpt-5-nano`와
`gpt-5-mini`를 human-approved 소량 조건으로 비교하면서도 summary 결과는
report에만 남기고 DB write, raw extraction, API/K8s 변경 없이 품질 검증
경계를 유지했다.
...
- provider validation 실패 사유를 report 또는 운영 로그로 남기는 방식 검토
