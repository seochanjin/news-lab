...
- `app/utils/article_embeddings.py`
  ...
  - 예상 token 수 계산
  - 승인 Fix 1에 따라 empty provider input 무호출 처리와 응답 embedding
    개수 검증 추가
...
- 실제 provider 호출 전 예상 article 수, token 수, 비용을 stdout에
  출력하도록 구성했다.
- 승인 Fix 1 적용 후 OpenAI provider는 입력을 먼저 list로 고정하고,
  empty input이면 HTTP 요청 없이 `[]`를 반환한다.
- Provider 응답 `data`와 정렬된 embedding 결과 개수가 입력 text 개수와
  다르면 `RuntimeError`를 발생시켜 article과 vector의 위치 불일치를
  방지한다.
...
- **OpenAI provider batching/chunking 즉시 구현**
  - 운영 규모 확장에는 필요하지만 현재 실제 provider 호출 상한이
    200건이다. 승인 Fix 1의 empty input/응답 개수 검증만 적용하고,
    batching은 이번 PR에 적용하지 않은 deferred 상태로 유지했다.
...
- 실제 provider는 opt-in, API key, 명시 article limit, 최대 200건 조건을
  ...
  우발적으로 수행하지 않기 위해서다.
- Provider 호출 경계에서 empty input과 응답 개수 불일치를 즉시
  처리하도록 했다. 불필요한 요청과 article/vector 매핑 오류를 grouping
  단계 전에 차단하기 위해서다.
...
- 현재 OpenAI provider는 최대 200건을 한 HTTP 요청으로 처리한다. 상한을
  ...
  필요하다.
- Provider 응답 개수 검증은 누락·초과 응답을 차단하지만 embedding
  dimension이나 index 중복까지 검증하지는 않는다. 실제 provider 운영
  단계에서 추가 검증 정책을 검토할 수 있다.
...
- Python compile 통과
- 승인 Fix 1 적용 후 전체 unittest 36개 통과
- 테스트는 fake/hash embedding과 mocked HTTP response만 사용했으며 외부
  API를 호출하지 않았다.
- Empty provider input이 `requests.post`를 호출하지 않고 `[]`를 반환함을
  확인했다.
- Provider 응답 embedding이 입력보다 적거나 많으면 `RuntimeError`가
  발생하고, 정상 응답은 기존 index 순서로 정렬됨을 확인했다.
...
- 승인 Fix 1 적용 후 empty provider input은 외부 HTTP 요청 없이 `[]`를
  반환하고, 응답 embedding 개수 불일치는 `RuntimeError`로 차단된다.
- Fix 적용 후 전체 unittest 36개, Python compile, CLI help,
  `git diff --check`, K8s 범위와 security 검사가 통과했다.
- Fix가 mocked provider response 처리에만 영향을 주므로 read-only DB
  dry-run은 재실행하지 않았다.
...
승인 Fix 1을 통해 비용이 발생할 수 있는 provider 호출에서 empty input을
차단하고, 입력 article 수와 반환 embedding 수가 어긋난 상태로 clustering이
진행되는 것을 방지했다.
...
read-only JSON dry-run으로 pipeline을 검증했다. Provider empty input
무호출 처리와 응답 개수 검증을 추가하고, 전체 36개 unittest와
...
- 실제 provider 운영 전 embedding dimension, response index 중복,
  malformed response 검증 필요성을 검토한다.
