...
- Runbook에 human-controlled DNS, dry-run, apply, Certificate/ACME, HTTPS
  검증 절차 기록
- 승인된 review fix 반영
  - 기능 변경 파일과 workflow 문서 범위를 구분해 scope 표현 보정
  - YAML 검증 command를 Python `yaml.safe_load_all` 기반으로 교체
...
실제 운영 동작을 변경하는 파일은 `k8s/news-api.yaml`이다.
`docs/ARCHITECTURE.md`와 `docs/RUNBOOK.md`는 구조와 운영 절차를 반영하며,
task, verification, PR, devlog, fixes, review 문서는 요구사항과 검증 근거,
승인된 fix 이력을 관리한다.

승인된 후속 fix에서는 multi-document Kubernetes YAML 검증 command를 Ruby
`YAML.load_stream`에서 Python `yaml.safe_load_all`로 교체했다. 이 command는
`---`로 구분된 `Deployment`, `Service`, `Ingress`를 하나의 파일에서 모두
읽되 안전한 loader를 사용한다.
...
- 초기 전체 `k8s/*.yaml` YAML stream parsing: 통과
...
- `git diff --check`: 통과
- Python `yaml.safe_load_all` 기반 전체 `k8s/*.yaml` parsing: 통과
...
- approved-fix scope diff: task, fixes, review 문서 변경만 포함
- application code, scripts, tests, DB, Dockerfile, GitHub Actions, frontend,
  `.env`, `.env.*` 보호 범위 diff/status: 변경 없음
- security grep: 기존 안전한 참조와 command text만 확인됐으며 신규
  credential 또는 secret 값 없음
...
승인된 review fix 두 건은 문서 정합성과 검증 command 개선으로 적용됐다.

- 실제 기능 변경과 workflow artifact 범위를 구분하도록 scope 표현 수정
- YAML parsing command를 Python `yaml.safe_load_all` 기반으로 교체

두 fix는 Kubernetes manifest의 동작을 변경하지 않는다. PR merge,
production deployment, K3s rollout, TLS 발급 및 HTTPS 운영 검증 완료를
주장하지 않는다.
...
- 승인된 scope 문구와 Python multi-document YAML 검증 command가
  반영됐으며 재검증을 통과했다.
...
후속 frontend 전환을 단계적으로 분리한 작업이다. 후속 review에서는 실제
기능 변경과 workflow 문서 범위를 명확히 구분하고, multi-document manifest
검증을 Python safe loader로 표준화했다.
