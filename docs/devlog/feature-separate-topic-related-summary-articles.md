...
관련 기사는 기존 대표 후보 순위를 유지한 채 Topic별 상한까지 선택한다. Summary 근거 기사는 이 순서에서 대표 기사를 포함하고 URL 또는 공백·대소문자를 정규화한 제목 중복을 제거한 뒤 상한까지 선택한다. ID를 마지막 tie-breaker로 사용해 같은 입력에 대해 결정론적인 결과가 나오도록 했다.
...
- URL·제목 중복 제거는 현재 기사 metadata 품질에 의존한다.
...
- approved fixes 문서에는 승인된 fix와 적용 내역이 없어 추가 fix를 반영하지 않았다.
- 전체 자동 테스트 201건이 통과했다.
...
뉴스 Topic pipeline에서 관련 기사 보존 집합과 LLM Summary 근거 집합을 분리했다. 관련 기사는 Topic별 최대 20건까지 기존 API에 노출하면서, 원문 추출과 Summary provider 입력은 중복 제거된 최대 3건으로 제한했다. 단계 간 dataclass 계약으로 부분집합 불변 조건을 검증하고, DB/API schema 변경 없이 저장 관계와 집계 기준을 확장했다. CLI 하위 호환성, CronJob 설정, 운영 문서, 201건의 전체 회귀 테스트까지 함께 정리했다.
