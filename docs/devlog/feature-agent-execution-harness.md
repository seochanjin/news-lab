...
- Verification이 failed/pending이어도 실행할 수 없는 Review action을 다시
  권장할 수 있었다.
- `docs/tasks/main.md` 본문에 Task 파일명만 포함돼도 정상 pointer로 오인할 수
  있었다.
- 환경변수로 지정한 Agent binary와 repository 외부 로그 경로의 유효성 검증이
  부족했다.
- Review 문서의 `~/news-lab/...` 링크가 작성자의 로컬 환경에 종속됐다.
- 외부 review에 기록된 테스트 수와 로컬 Verification 결과가 달라 실행 환경과
  실제 수집 수를 다시 확인할 필요가 있었다.
...
- failed/pending Verification은 `resolve-verification` 안내를 출력하고 Review
  gate를 통과하지 못하도록 상태와 action 계약을 맞췄다.
- `docs/tasks/main.md`는 실제 Markdown link target이 현재 Task와 정확히
  일치할 때만 pointer로 인정하도록 강화했다.
- 환경변수 Agent binary의 존재·실행 가능 여부를 확인하고 repository 외부
  로그 경로도 안전하게 직렬화하도록 보완했다.
- Antigravity Review의 로컬 home 경로 링크를 저장소 상대 Markdown link로
  정규화했다.
...
- 명시적 `Verification Status: pending` 상태에서도 Review 차단
- Review가 이미 존재하는 failed/pending 상태에서는 `resolve-verification`
  안내 출력
- 환경변수 binary가 실행 가능한 파일 또는 PATH command인지 검증

Gate는 실행 가능 여부만 판정하며 production command를 대신 실행하지 않는다.

### Task pointer 검증

`docs/tasks/main.md`의 파일명 문자열 포함 여부가 아니라 fenced code block 밖의
실제 Markdown link target을 추출한다. pointer 문서 기준으로 경로를 정규화한
뒤 현재 branch의 Task 경로와 정확히 일치할 때만 통과한다.

일반 본문에 Task 파일명이 있거나 코드 예시에 정상 link가 있어도 pointer로
인정하지 않는다.
...
환경변수로 Agent binary를 지정하면 경로는 실행 가능한 일반 파일인지,
command 이름은 PATH에서 찾을 수 있는지 확인한다. 잘못된 경로와 디렉터리는
환경변수 값을 노출하지 않는 한글 오류로 차단한다.

로그 경로가 repository 내부이면 상대 경로를, 외부이면 정규화된 절대 경로를
`result.json`에 기록한다. 경로 표시 실패가 Agent 실행 결과 저장 실패로
이어지지 않게 했다.
...
- FIX-04: Python 3.11.7, pytest 9.1.1 환경에서 테스트를 재실행해 외부 review의
  100 tests / 12 errors가 재현되지 않음을 확인하고 실제 수를 문서에 반영했다.
- FIX-05: failed/pending Verification과 다음 action 및 Review gate의 계약을
  일치시켰다.
- FIX-06: `main.md`의 실제 Markdown link target을 검증하도록 강화했다.
- FIX-07: 환경변수 Agent binary의 존재와 실행 가능 여부를 검증했다.
- FIX-08: repository 외부 로그 경로를 절대 경로로 안전하게 직렬화했다.
- FIX-09: Review의 로컬 경로 링크를 저장소 상대 링크로 변경했다.
...
### Task 파일명 문자열 포함으로 pointer 판정

구현은 간단하지만 일반 설명과 코드 예시를 실제 pointer로 오인할 수 있다.
Markdown link target을 추출하고 정규화된 경로를 비교하는 방식을 선택했다.

### 환경변수 binary를 그대로 신뢰

유연하지만 typo, 디렉터리 또는 실행 권한이 없는 경로가 subprocess 단계에서
늦게 실패한다. gate에서 실행 전에 검증하도록 변경했다.

### 외부 로그 경로 금지

항상 repository 내부 로그만 허용하면 구현은 단순하지만 테스트와 별도 보관
경로 사용성이 떨어진다. 내부·외부 경로를 모두 지원하고 표시 형식만 구분했다.
...
- 상태가 failed/pending이면 실행 불가능한 action을 제안하지 않고 사람이 먼저
  해결할 작업을 명시적으로 출력하도록 했다.
- Task pointer, Agent binary와 로그 경로는 실제 실행 직전에 실패하기보다
  deterministic한 gate와 경로 정규화로 조기에 검증하도록 했다.
...
- Python docstring 정책은 이해 가능성을 높이지만 변경 시 문서 유지 비용이
  증가한다.
- Markdown link parser는 일반적인 inline link 형식을 대상으로 하며 복잡한
  reference-style Markdown 전체를 해석하는 범용 parser는 아니다.
- `resolve-verification`은 실행 action이 아니라 사람이 Verification을
  정리하도록 안내하는 상태 값이다.
- repository 외부 로그 절대 경로는 로컬 result JSON에 남으므로 로그 공유 시
  환경 종속 경로가 포함될 수 있다.
...
- `python -m pip install -r requirements-dev.txt`
  - 모든 dependency가 이미 설치된 상태임을 확인
  - pyenv shim rehash 디렉터리 권한 문제로 command는 exit 1
  - 신규 설치나 dependency 변경은 없었고 같은 Python 환경의 전체 test는 통과
...
CodeRabbit에 기록된 100 tests / 12 errors는 현재 실행 환경에서 재현되지
않았다. 실제 최종 결과는 Verification의 187 passed / 187 tests OK다.
...
- failed/pending Verification의 해결 안내와 Review gate 차단 확인
- `main.md` 정상 link, 잘못된 link, 일반 본문 및 code example 판정 확인
- 정상·잘못된 Agent binary 환경변수 검증 확인
- repository 내부·외부 로그 경로 직렬화 확인
- Review 문서의 로컬 경로 link 제거 확인
...
호환했다. 명시적 Verification 상태 모델과 187개 회귀 테스트를 통해 과거 로그
문자열 오탐, 상태와 action 계약 불일치, 부정확한 Task pointer, 잘못된 Agent
binary, 외부 로그 경로, unsafe branch 실행과 비정상 subprocess 종료를
...
- pytest가 없는 환경에서는 자동 설치가 아니라 `requirements-dev.txt` 설치
  안내를 출력하는 기능을 검토한다.
- pyenv shim rehash 권한 문제는 프로젝트 코드 문제가 아니므로 필요하면 로컬
  Python 환경 권한을 사람이 별도로 정리한다.
