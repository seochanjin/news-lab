...
- 승인된 FIX-01을 적용해 exit code 0인 정상 process는 출력에 실패 marker가
  포함되어도 failure category로 오분류하지 않도록 했다.
- 승인된 FIX-02를 적용해 외부 Gemini
  `reasonCode: UNSUPPORTED_CLIENT`와 내부 `unsupported_client` category를
  구분하고 `automatic_review_unavailable`을 문서의 category 목록에 포함했다.
...
승인된 fix 적용 후 failure 분류 순서는 다음과 같다.

1. timeout이면 `timeout`
2. exit code가 0이면 출력 marker와 관계없이 정상 process
3. non-zero process에서만 stdout·stderr marker를 해석
4. 알려진 marker가 없으면 `nonzero_exit`

따라서 정상 출력에 “unsupported client 문제가 해결됐다”거나 과거 인증 실패를
설명하는 문장이 있어도 실패로 오분류하지 않는다.
...
### 과거 자동 review 실패를 근거로 후속 단계를 영구 차단

자동 실행 실패 뒤 review가 미완성이면 진행을 차단해야 하지만, 이후 사람이 수동
review를 완성하고 Approved Fixes를 승인해도 영구 차단하면 fallback으로 복구할
수 없다. 따라서 과거 실패가 아니라 현재 review 검증과 Approved Fixes 상태를
진행 기준으로 유지했다.

### Failure category별 retry 정책을 함께 구현

인증 실패 재시도 금지, timeout 재시도, backoff와 반복 실패 보존은 별도 retry
engine 설계가 필요하다. 현재 Task의 실패 분류와 수동 fallback 범위를 넘으므로
deferred로 남겼다.

### `WorkflowState` type-checking import 추가

현재 annotation은 runtime에서 안전하고 실제 type checker 실패도 확인되지
않았다. 정적 type checker를 도입해 unresolved-name 문제가 확인될 때
`TYPE_CHECKING` import를 검토하기로 했다.
...
- 정상 process는 marker를 해석하지 않으므로 exit code 0을 잘못 반환하는 외부
  adapter의 오류 문구는 failure category만으로 감지할 수 없다. 대신 자동
  review는 후속 review 파일 생성·변경·검증도 통과해야 완료된다.
...
- 과거 자동 review 실행 실패만으로 후속 단계를 영구 차단하지 않는다. 현재
  review 파일 검증과 Approved Fixes 상태가 복구 여부를 결정한다.
...
- 승인 FIX-01 runner 전용 검증
  - 11개 테스트와 6개 subtest 통과
- 승인 FIX-02 문서 정합성 검색
  - 외부 `UNSUPPORTED_CLIENT`, 내부 `unsupported_client`,
    `automatic_review_unavailable` 구분 확인
- 승인 fixes workflow 회귀
  - 45개 테스트와 11개 subtest 통과
...
- exit code 0인 정상 process는 실패 marker가 포함된 설명 문장 때문에 실패로
  오분류되지 않는다.
- 외부 `reasonCode: UNSUPPORTED_CLIENT`와 내부 `unsupported_client` category가
  문서에서 구분된다.
- Codex implement, `codex-fix`, Task parser와 UNIT 실행 흐름에 회귀가 없었다.
- application, API, DB, Daily Topic pipeline과 K3s 영역은 수정하지 않았다.
- 승인된 FIX-01과 FIX-02가 적용되고 관련 검증과 전체 회귀가 통과했다.
- review와 review validation은 `completed`, Approved Fixes는 `applied`로
  판정된다.
...
외부 AI review CLI의 실행 파일 탐지와 실제 자동 실행 지원 여부를 분리한 안전한 workflow adapter를 설계했다. 지원되지 않는 Gemini client와 미검증 `agy` 실행을 성공으로 간주하지 않고 수동 fallback으로 전환했으며, process 실패와 review 문서 실패를 10개 category로 구분했다. 최초 review와 최신 Re-review의 필수 구조, 본문, 허용 Verdict를 검증하는 읽기 전용 parser를 추가하고 미완성 결과에서 fix·PR 단계 진행을 차단했다. 정상 process의 출력 marker 오분류 결함과 failure category 문서 정합성을 승인 fix로 보완했으며, 전체 223건 회귀 테스트를 통해 기존 Codex 실행과 application 영역에 회귀가 없음을 확인했다.

## 다음 단계 후보

- 사람이 현재 diff와 PR 초안을 확인한 뒤 commit, push와 PR 생성을 수행한다.
...
- failure category별 retry 허용, backoff와 반복 실패 보존이 필요하면 별도
  workflow 정책 Task로 설계한다.
- 정적 type checker를 도입해 annotation 오류가 확인되면
  `WorkflowState`의 `TYPE_CHECKING` import를 검토한다.
