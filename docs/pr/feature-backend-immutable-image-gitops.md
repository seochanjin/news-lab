...
- 문서
  ...
  - review instruction에 review artifact가 local absolute path나 local URI를
    기록하지 않도록 repository-relative link 규칙을 추가했습니다.
...
- `docs/reviews/` 문서는 review/approved-fix workflow artifact이며, verification
  ...
  사용했습니다.
- 최종 review gate는 CodeRabbit review와 사람 검토입니다. Antigravity review
  artifact는 과거 검토 기록으로만 유지하며 최종 승인 증거로 사용하지 않습니다.
