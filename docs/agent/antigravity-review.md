...
자동 UNIT Review는 Agent process 성공, stdout 응답 검증, append-only writer
성공을 모두 통과해야 완료다. 수동 review는 자동 실행 기록 없이도 기존 수동
구조 검증을 통과하면 완료다. Review finding은 사람이 Approved Fixes에
승인하기 전까지 구현 변경 근거가 아니다.
