...
Antigravity의 표시 이름, 실제 adapter, 실행 파일과 자동 실행 지원 여부는 별도
상태다. `gemini` 또는 `agy` 실행 파일이 설치되어 있다는 사실만으로 자동 review를
지원한다고 판정하지 않는다. 비대화형 prompt, 인증, permission, 파일 작성과
종료 상태 계약이 실제로 검증된 adapter만 subprocess로 실행할 수 있다.

현재 기본 상태는 `automatic review unavailable`과 `manual review required`다.
이 경우 다음 prompt-only 흐름을 사용한다.
