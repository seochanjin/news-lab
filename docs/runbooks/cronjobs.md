...
KUBECONFIG=~/.kube/oci-k3s.yaml kubectl create job \
  ...
  news-daily-topic-pipeline-prewarm-verify \
  -n default
KUBECONFIG=~/.kube/oci-k3s.yaml kubectl wait \
  --for=condition=complete \
  job/news-daily-topic-pipeline-prewarm-verify \
  -n default \
  --timeout=30m
...
- Daily Pipeline Job이 `Complete`이고 log에 `operation=prewarm` 성공 또는
  fail-open 사유가 credential 없이 기록된다.
- 첫 `/topics/home` 요청 전에 `EXISTS topics:home:v1` 결과가 `1`이다.
- Pipeline 직후 TTL은 약 `108000`초다.
- 첫 `/topics/home` 요청 후 TTL은 감소하며 `108000`초로 다시 초기화되지 않는다.
...
KUBECONFIG=~/.kube/oci-k3s.yaml kubectl create job \
  ...
  news-three-day-topic-pipeline-prewarm-verify \
  -n default
KUBECONFIG=~/.kube/oci-k3s.yaml kubectl wait \
  --for=condition=complete \
  job/news-three-day-topic-pipeline-prewarm-verify \
  -n default \
  --timeout=30m
KUBECONFIG=~/.kube/oci-k3s.yaml kubectl logs \
  job/news-three-day-topic-pipeline-prewarm-verify \
...
KUBECONFIG=~/.kube/oci-k3s.yaml kubectl create job \
  ...
  news-weekly-topic-pipeline-prewarm-verify \
  -n default
KUBECONFIG=~/.kube/oci-k3s.yaml kubectl wait \
  --for=condition=complete \
  job/news-weekly-topic-pipeline-prewarm-verify \
  -n default \
  --timeout=30m
KUBECONFIG=~/.kube/oci-k3s.yaml kubectl logs \
  job/news-weekly-topic-pipeline-prewarm-verify \
