...
Backend image 배포 기준은 full Git SHA다. GitHub Actions가 image build 성공 후
manifest update bot PR을 생성하고, 사람이 manifest PR diff를 검토해 merge한다.
그 뒤 사람이 Argo CD `news-api` diff를 확인하고 Manual Sync를 승인한다. Auto
sync, automatic prune, automatic self-heal은 사용하지 않는다.

Rollback도 이전 정상 full SHA를 manifest에 반영하는 PR, merge, Manual Sync로
수행한다. `latest`나 `kubectl rollout restart`만으로 image version을 변경하지
않는다.
...
## UNIT-01 immutable 전환 전 수동 배포 흐름

NewsLab의 immutable image 전환 전 배포 흐름은 image 발행과 K3s 반영이 분리되어
있었다.
...
push된다. workflow는 full Git SHA tag와 `latest` tag를 발행하지만, 전환 전
운영 manifest는 `seocj/news-api:latest`를 참조했다.
...
전환 전 backend 배포에서 사람이 판단하던 대표 작업은 다음이다.
...
Backend 운영 manifest는 `latest` 대신 Git commit을 식별할 수 있는 full Git SHA
image tag를 사용한다.
...
- Backend manifest tag 갱신: image build 성공 후 GitHub Actions가 manifest update
  bot PR 생성
- 운영 반영: 사람이 manifest PR diff를 검토하고 merge한 뒤 Argo CD Manual Sync 승인

Frontend는 현재 `sha-<short-sha>` tag를 발행하므로, full Git SHA tag 발행은
별도 GitHub Actions 변경 task에서 다룬다.
...
Backend Argo CD 설치, `news-api` Application 등록, Backend immutable image 전환,
운영 Manual Sync, controlled rollback과 restore 검증은 완료되었다. 남은 운영
개선은 다음 별도 task로 분리한다.

1. Frontend Application 등록
   - `news-lab-web` Application은 Manual Sync로 등록한다.
2. Frontend Manual Sync 검증
   - diff, Sync, rollout, web endpoint, Ingress/Middleware 상태를 사람이 확인한다.
3. Frontend 고정 image tag 적용
   - Frontend tag 형식과 workflow 변경 여부를 별도 PR로 다룬다.
4. Argo CD 접근 방식의 Tailscale 내부화 검토
5. README GitOps 배포 구조 반영 여부 검토

위 단계의 command 예시는 후속 runbook에서 human-controlled command로만 작성할
수 있다. 실행하지 않은 후속 작업 결과를 완료된 운영 결과처럼 기록하지 않는다.
