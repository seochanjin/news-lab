...
→ PostgreSQL 저장 성공 후 topics:home:v1 prewarm
...
Execute 결과에서 실제 DB write가 성공한 경우에만 새 read connection으로 Home
payload를 다시 만들고 `topics:home:v1`을 overwrite한다. Dry-run이나 DB write가
없었던 실행은 prewarm을 건너뛴다.
...
→ PostgreSQL 저장과 run 종료 성공 후 three-day-topics:home:v1 prewarm
...
Publishable Topic을 한 건 이상 저장하고 run을 `success` 또는
`partial_success`로 종료한 뒤에만 최신 PostgreSQL payload로
`three-day-topics:home:v1`을 overwrite한다. Dry-run과 저장 결과가 없는 실행은
prewarm을 건너뛴다.
...
직전 완료 주간 articles
→ 기존 article_embeddings 호환성·source hash 확인
→ 저장 vector가 있는 기사만 7일 기준 재클러스터링
→ 최소 기사 5개·최소 source 2개 Topic 후보 필터
→ 대표/관련/Summary 근거 기사 선정
→ Summary 근거 기사의 저장 원문 재사용 또는 지연 추출
→ 지난 월요일-일요일 흐름을 설명하는 Summary 생성
→ weekly_topics / weekly_topic_articles 원자 교체
→ weekly_topic_runs 실행 이력
→ PostgreSQL 저장과 run 종료 성공 후 weekly-topics:home:v1 prewarm
→ /weekly-topics, /weekly-topics/home, /weekly-topics/{topic_id}
```

진입점은 `scripts/run_weekly_topic_pipeline.py`다.
`news-weekly-topic-pipeline` CronJob은 일요일 전체 기사가 수집된 뒤인
`00:30 Asia/Seoul` 매주 월요일에 실행되도록 manifest에 정의되어 있다.

Pipeline 시작 시 `Asia/Seoul` 기준 완료된 calendar week를 한 번 확정한다.
기본 실행은 실행 시점보다 앞선 가장 최근 완료 주간을 선택하고, 명시
`--week-start`는 `YYYY-MM-DD` 형식의 월요일만 허용한다. 모든 단계와 저장 row는
같은 `week_start`, `week_end`, `[window_start, window_end)`를 사용한다.

Weekly pipeline은 기존 `article_embeddings`를 읽기만 한다. 신규 embedding
provider 호출, embedding insert/update와 Daily 또는 3일 Topic 결과 재집계는
수행하지 않는다. 누락되거나 현재 title/summary hash와 맞지 않는 embedding은
기사 단위로 제외하고 누락 통계에 반영한다.

원문 확보와 Summary 실패는 Topic 단위로 격리한다. 일부 Topic만 성공하면 성공
부분집합을 저장하고 run을 `partial_success`로 기록한다. 전부 실패하면 기존
window 결과를 유지하며, 정상적으로 선정 Topic이 없는 실행은 빈 결과로 window를
원자 교체할 수 있다.

CLI 기본값은 dry-run이다. `--execute --use-summary-provider`에서만 run 이력 생성,
지연 원문 추출, Summary provider 호출과 결과 교체가 발생한다. 세부 계약과
대안은 [7일 Topic 설계 문서](../design/weekly-topic-pipeline.md), 운영 절차는
[CronJob runbook](../runbooks/cronjobs.md)을 참고한다.

Publishable Topic을 한 건 이상 저장하고 run을 `success` 또는
`partial_success`로 종료한 뒤에만 최신 PostgreSQL payload로
`weekly-topics:home:v1`을 overwrite한다. Dry-run과 저장 결과가 없는 실행은
prewarm을 건너뛴다.

## Home Cache prewarm 공통 정책

Daily·3-day·Weekly Pipeline의 prewarm은 Home API cache miss와 같은 PostgreSQL
payload builder를 사용하되 Redis GET 없이 대응 key를 overwrite한다. Daily와
3-day TTL은 `108000`초, Weekly TTL은 `691200`초다. TTL은 최신성 주기가 아니라
오래된 cache의 장기 잔류를 제한하는 안전장치다.

Redis 미설정, 연결 실패, timeout과 SET 실패는 Pipeline 결과를 실패로 바꾸지
않는 fail-open 정책을 사용한다. PostgreSQL/Supabase가 영속 Source of Truth이며,
Pipeline이 PostgreSQL에서 payload를 읽어 Redis에 쓰므로 두 저장 계층이 서로
직접 통신하지 않는다. API 조회 경로와 cache event는
[FastAPI와 API 영역](backend-api.md)을 참고한다.
