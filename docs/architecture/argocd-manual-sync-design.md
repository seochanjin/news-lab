...
`Ingress/news-api-ingress`, `Deployment/news-redis`, `Service/news-redis`와 네
Backend CronJob이다. Application 생성 전 rendered resource 목록에
`ClusterIssuer`가 없음을 확인해야 하며, live `ClusterIssuer`의 존재와 현재 관리
주체도 사람이 확인한다. 제외 설정이 동작하지 않거나 ownership이 불명확하면
Application 생성과 Sync를 중단한다.
...
`cluster-issuer.yaml`은 directory exclude로 별도 제외한다. 따라서 Redis cache
도입 후 기대하는 generated resource는 Backend resource 9개다.
...
- 관리 대상은 Backend Deployment, Service, Ingress, Redis Deployment/Service와
  네 CronJob 등 9개다.
...
## Backend immutable image GitOps 운영 기준

Backend `news-api`는 full Git SHA image tag를 운영 manifest의 기준으로 사용한다.
GitHub Actions는 `seocj/news-api:${{ github.sha }}` image build/push에 성공한 뒤
`bot/update-news-api-image-${{ github.sha }}` branch와 manifest update PR을
생성한다. 사람이 manifest PR diff를 검토하고 merge하면 Argo CD가 `main` revision
변경을 `OutOfSync`로 감지한다.

Argo CD `news-api` Application은 Manual Sync 정책을 유지한다.
`spec.syncPolicy.automated`를 두지 않으며, auto sync, automatic prune,
automatic self-heal은 적용하지 않는다. Sync 전 diff는 Deployment 1개와 CronJob
4개의 image 변경으로 제한되는지 확인한다. Sync 후에는 rollout, Pod Ready,
Deployment와 CronJob image 일치, Service/Ingress 회귀 여부와 production
`/health`를 확인한다.

Rollback도 같은 GitOps 경로를 사용한다. 이전 정상 full SHA를 manifest에 반영하는
PR을 만들고, 사람이 merge와 Argo CD Manual Sync를 승인한다. 최신 SHA로 복원할
때도 동일하게 restore PR과 Manual Sync를 사용한다. `latest`나
`kubectl rollout restart`만으로 image version을 변경하지 않는다.

## UNIT-01 immutable 전환 전 배포 baseline
...
immutable image 전환 전 흐름은 다음과 같았다.
...
- 전환 전 운영 manifest의 실제 image reference:
  - `seocj/news-api:latest`

Backend workflow는 full Git SHA tag와 `latest` tag를 함께 발행한다. 그러나
immutable image 전환 전 K3s manifest는 full Git SHA tag가 아니라 `latest`만
참조했다.
...
이 baseline 시점의 backend와 frontend는 각 repository가 자신의 Kubernetes manifest를
보유한다.
...
immutable image 전환 전 사람이 승인하고 실행하던 지점은 다음과 같다.
...
Git manifest의 image tag 갱신은 image build 성공 이후 GitHub Actions가 생성하는
manifest update PR로 수행한다. CI가 `main`에 직접 push하지 않고 별도 branch와 PR을
만들며, 사람이 build 결과의 Git SHA tag와 manifest diff를 확인한 뒤 merge한다.
이 방식은 반복 작업을 줄이면서도 운영 desired state 변경 이력을 Git PR로 남긴다.
...
Production verification은 사람이 수행하고 결과를 Verification 문서에 기록한다.
Argo CD 상태만으로 완료 처리하지 않는다.
...
최초 Backend 설치와 Manual Sync, Backend immutable image 전환, controlled
rollback/restore 검증은 완료되었다. 남은 운영 변경은 별도 task로 분리한다.

1. Frontend Application 등록 및 Manual Sync 검증
2. Frontend 고정 image tag 적용
3. Argo CD 접근 방식의 Tailscale 내부화 검토
4. README GitOps 배포 구조 반영 여부 검토
