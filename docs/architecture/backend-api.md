...
`/topics/home`의 Redis cache key는 `topics:home:v1`, 초기 TTL은 60초다. Redis는
source of truth가 아니며 Redis 미설정, 연결 실패, timeout, payload 손상은
PostgreSQL fallback으로 처리한다. Cache 상태는 response body가 아니라
`home_topics_cache event=hit|miss|store|bypass` 로그로 구분한다.
