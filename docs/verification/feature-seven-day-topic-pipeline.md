...
Result:
48 tests passed, 13 subtests passed in 0.44s. Approved Fix 적용 후 Weekly Home API
publishable status filter, 서울 기준 월요일 00:00 window 검증, raw acquisition 상태
bucket 상호 배타성, raw text loader 병합, run status/count 정합성, migration 정적
제약과 CronJob `automountServiceAccountToken: false` 계약을 집중 검증했다.

Status: passed

Command:
`python -m pytest tests/test_run_weekly_topic_pipeline.py tests/test_weekly_topic_pipeline.py tests/test_weekly_topic_repository.py tests/test_weekly_topics_api.py tests/test_weekly_topic_pipeline_cronjob_manifest.py -v`

Result:
57 tests passed, 13 subtests passed in 0.38s. Weekly 실행 진입점 포함 집중 테스트가
통과했다.

Status: passed

Command:
`python -m pytest tests/test_run_daily_topic_pipeline.py tests/test_daily_topic_pipeline_cronjob_manifest.py tests/test_daily_topic_article_selection.py tests/test_daily_topic_pipeline_configuration.py -v`

Result:
34 tests passed in 0.19s. Approved Fix 적용 후 기존 Daily Topic runner, CronJob,
기사 선정과 설정 계약 회귀를 확인했다.
...
Result:
33 tests passed, 6 subtests passed in 0.37s. Approved Fix 적용 후 기존 3일 Topic
runner, pipeline, API와 CronJob 계약 회귀를 확인했다.
...
Result:
407 tests ran in 13.872s, OK. argparse 오류 메시지와 provider 실패 로그는 예상
실패 경로를 검증하는 테스트 출력이다.
...
Command:
`rg -n "jsonb_typeof|article_count|source_count|weekly_topic_id, rank|is_representative|status|automountServiceAccountToken|시용 날짜|표시용 날짜|draft|publishable|weekly-topics/home|Asia/Seoul|00:00" db/migrations/008_create_weekly_topic_tables.sql tests/test_weekly_topic_repository.py k8s/news-weekly-topic-pipeline-cronjob.yaml tests/test_weekly_topic_pipeline_cronjob_manifest.py docs README.md app/routers/weekly_topics.py app/services/weekly_topic_pipeline`

Result:
Weekly migration의 JSONB array CHECK, 최소 기사/source count CHECK,
`(weekly_topic_id, rank)` UNIQUE, 대표/Summary evidence CHECK, Topic status CHECK,
CronJob `automountServiceAccountToken: false`, `/weekly-topics/home`의 publishable
status filter, `Asia/Seoul` 월요일 `00:00` window 문구와 테스트 참조를 확인했다.
`시용 날짜` 오타는 검색 결과에 없고, `표시용 날짜` 문구가 task에 존재한다.
...
Result:
첫 실행은 작업 시작 전부터 modified 상태였던
`docs/reviews/feature-seven-day-topic-pipeline-coderabbit.md`의 줄 끝 공백 때문에
실패했다. Review 내용은 변경하지 않고 줄 끝 공백만 제거했다.
...
Result:
Approved Fix 적용 범위는 Weekly API/router, Weekly pipeline model/raw/summary,
Weekly migration, Weekly CronJob manifest, 관련 Weekly 테스트와 설계/PR/devlog/fix
문서다. 기존 Daily runner/CronJob, 3일 runner/CronJob과 기존 3일 migration은 diff가
없다.

Status: passed

Command:
`git diff -- db/migrations/007_create_three_day_topic_tables.sql scripts/run_daily_topic_pipeline.py scripts/run_three_day_topic_pipeline.py k8s/news-daily-topic-pipeline-cronjob.yaml k8s/news-three-day-topic-pipeline-cronjob.yaml`
...
- Weekly model은 월요일 시작 완료 주간, 서울 기준 월요일 00:00부터 다음 월요일
  00:00까지의 정확한 7일 window, 최소 기사 5개, 최소 source 2개, Summary 근거
  최대 5개, 대표 기사 1건, raw acquisition 상태 bucket 상호 배타성,
  run status/count 정합성과 Topic publish status를 저장 전에 검증한다.
...
- `/weekly-topics/home`은 성공 또는 부분 성공 run이 만든 최신 완료 주간 window의
  `ready` Topic card payload만 반환하며, `ready` Topic이 없는 window는 최신 공개
  window 선택 대상에서 제외한다. 정적 `/home` route는 동적 `/{topic_id}` route보다
  먼저 등록된다.
...
- Weekly CronJob은 기존 image, `DATABASE_URL`, `OPENAI_SUMMARY_API_KEY`,
  ...
  CronJob과 맞추며 `OPENAI_EMBEDDING_API_KEY`를 주입하지 않는다. Kubernetes API를
  직접 호출하지 않으므로 service account token 자동 mount를 비활성화한다.
...
- Approved Fix 적용으로 Weekly migration의 JSONB array, 최소 기사/source count,
  Topic status, Topic 내 rank, 대표/Summary evidence 관계, similarity 범위,
  단순 날짜/run 시간 관계 제약을 application model과 맞췄다.
