...
## Approved Fix Verification: CodeRabbit Runtime and Review Hygiene

Commands run for CodeRabbit approved fixes:

```bash
git status --short --branch
sed -n '1,240p' AGENTS.md
sed -n '1,260p' docs/prompts/codex-implement.md
sed -n '1,420p' docs/fixes/feature-rss-source-db-ingest-approved-fixes.md
sed -n '1,360p' docs/tasks/feature-rss-source-db-ingest.md
...
Applied CodeRabbit approved fixes:
...
The collector was not rerun for this CodeRabbit fix because `scripts/collect_rss.py` writes to the configured database and the approved fix removed data-writing collector execution from default review verification.
