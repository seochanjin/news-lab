...
- [x] Production 배포·API smoke verification은 사람이 수행하고, DB migration이나 기존 row update가 없음을 sanitized evidence와 함께 Verification에 기록한다.
...
- Approved fixes 반영 후 확정된 targeted suite는
...
- 사람이 Backend PR #66과 GitOps image PR #67 merge, Argo CD
  `Synced / Healthy`, rollout 성공, Backend Pod `2/2 Ready`와 restart 0을
  확인했다. Deployment와 CronJob 4개는 같은 immutable image를 사용한다.
- Production health·version과 3-day·Weekly Home/list/detail API가 모두 HTTP
  200을 반환했다. Sanitized response 42건에서 invalid title, 잔존 날짜·기간
  pattern, missing period와 invalid period가 모두 0이고 validation exit가 0이었다.
- DB migration, 기존 DB row update, Pipeline·CronJob 수동 실행과 Secret
  변경·조회 없이 UNIT-05와 전체 Verification을 완료했다.
...
- [x] UNIT-05: Backend targeted·전체 회귀 테스트, 사람 주도 Production 배포·API 검증과 최종 문서화
