...
### Fix 6: Manifest structure test 보강

#### Background

CodeRabbit review에서 기존 manifest test가 substring 기반이라 `activeDeadlineSeconds`나 command가 잘못된 YAML 위치에 있어도 통과할 수 있다는 지적이 있었다.

#### Approved Change

`k8s/news-daily-topic-pipeline-cronjob.yaml`을 YAML로 parse해 batch/v1 CronJob의 실제 nested path를 검증한다.

검증 대상:

- `spec.jobTemplate.spec.activeDeadlineSeconds == 1800`
- `spec.jobTemplate.spec.template.spec.containers[0].command[:3] == ["python", "-u", "scripts/run_daily_topic_pipeline.py"]`
- 기존 bounded args와 Secret reference 유지

#### Deferred

non-root execution은 별도 hardening 작업으로 분리한다.
...
- `tests/test_daily_topic_pipeline_cronjob_manifest.py`
  - YAML parse 기반 nested CronJob 구조 검증으로 보강
  - 30분 deadline과 unbuffered command prefix를 실제 nested path에서 검증
  - 기존 bounded command, Secret reference, resource/security, schedule 검증 유지

- `requirements.txt`
  - 별도 test dependency 패턴이 없어 parser 기반 committed test 재현을 위한
    최소 dependency로 `PyYAML` 추가
