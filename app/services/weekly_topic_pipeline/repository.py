"""7일 Topic 실행 이력과 활성 주간 결과를 PostgreSQL에 저장한다.

Repository는 주입받은 SQLAlchemy engine으로 실행 이력 transaction과 결과 교체
transaction을 분리한다. 결과 교체 시 window advisory lock을 획득한 뒤 기존
Weekly Topic 삭제와 신규 Topic·기사 관계 삽입을 한 transaction에서 수행한다.
후보 조회, provider 호출, migration 적용은 담당하지 않는다.
"""
...
insert into weekly_topic_articles (
    weekly_topic_id,
    article_id,
    rank,
    similarity,
...
class WeeklyTopicRepository:
    """Engine transaction을 사용해 7일 Topic run과 현재 결과 세대를 저장한다."""
    ...
    def create_run(self, run: WeeklyTopicRunStart) -> int:
        """`running` 주간 실행 이력을 별도 transaction으로 생성하고 ID를 반환한다."""
    ...
    def finish_run(
        self,
        run_id: int,
        completion: WeeklyTopicRunCompletion,
    ) -> None:
        ...
        with self._engine.begin() as connection:
            ...
            if result.rowcount != 1:
                raise ValueError(f"running weekly topic run not found: {run_id}")
    ...
    def replace_window_topics(
        ...
        *,
        ...
        window_end: datetime,
        topics: Sequence[WeeklyTopicRecord],
    ) -> list[int]:
        """동일 주간 window의 기존 Topic을 신규 결과로 원자적으로 교체한다.
...
def _validate_replacement_window(
    ...
    if topics:
        ...
        _validate_empty_replacement_window(window_start, window_end)
...
def _validate_empty_replacement_window(
    window_start: datetime,
    window_end: datetime,
) -> None:
    """빈 결과 교체 시에도 timezone-aware인 7일 window인지 확인한다."""
    ...
    if local_end.date() != local_start.date() + WEEKLY_WINDOW:
        raise ValueError("weekly topic window must end at the next Monday")
    if (
        local_end.hour != 0
...
def _window_lock_key(window_start: datetime, window_end: datetime) -> str:
    ...
    return f"weekly-topics:{start_utc}:{end_utc}"


def _topic_parameters(run_id: int, topic: WeeklyTopicRecord) -> dict:
    """Topic insert query에 전달할 bind parameter와 JSON 문자열을 구성한다."""

    return {
        "run_id": run_id,
        "week_start": topic.week_start,
        "week_end": topic.week_end,
