"""주간 흐름 Summary를 Topic별로 생성하고 성공 결과를 window 단위로 저장한다.

Summary 입력은 월요일부터 일요일까지의 기사 시각과 원문을 포함하며
`weekly-flow-v1` prompt 계약으로 hash를 계산한다. 원문 부족, 대표 기사 원문
누락과 provider 실패는 Topic별 실패로 격리하고, 성공 부분집합 또는 정상 빈
결과만 repository의 원자적 교체 transaction에 전달한다.
"""
...
PROMPT_VERSION = "weekly-flow-v2"
LOGGER = logging.getLogger(__name__)


class WeeklyOpenAISummaryProvider:
    """OpenAI Responses API에 주간 흐름 전용 prompt와 JSON schema를 전달한다."""
    ...
    def summarize(self, topic_input: dict) -> dict:
        """한 Topic 입력을 주간 prompt로 호출하고 검증된 Summary 객체를 반환한다."""
        ...
        response = requests.post(
            ...
            json={
                "model": self.model,
                "input": build_weekly_summary_prompt(topic_input),
                "text": {
                    "format": {
                        "type": "json_schema",
...
def summarize_and_persist_weekly_topics(
    ...
    *,
    ...
    """Topic별 주간 Summary를 만들고 저장 가능한 결과만 원자적으로 교체한다.

    Summary 후보 원문이 부족하면 같은 Topic의 다음 관련 기사 원문으로 대체한다.
    대표 기사 원문이 없거나 provider가 실패한 Topic은 실패 목록에 남기고 다음
    Topic을 계속한다. Execute 모드에서는 성공 Topic이 있거나 선택 Topic이 없는
    정상 빈 실행일 때만 동일 window 결과를 교체한다.
    """
    ...
    for topic in topic_result.selected_topics:
        try:
            ...
            LOGGER.warning(
                "weekly topic processing failed: topic_candidate_id=%s error=%s",
    ...
    return WeeklyTopicProcessingResult(
        topics=topics,
...
def build_weekly_summary_input(
    topic,
    raw_texts,
    *,
    max_raw_chars_per_article: int,
) -> dict:
    """한 Topic에서 원문이 있는 근거 기사를 시간순 주간 Summary 입력으로 구성한다."""
        ...
        used_articles.append(
            {
                ...
                "analysis_time": _isoformat_utc(
                    article.get("published_at") or article.get("analysis_time")
                ),
...
def build_weekly_summary_input_hash(summary_input: dict) -> str:
...
def build_weekly_summary_prompt(summary_input: dict) -> str:
    """OpenAI 계열 provider에 전달할 주간 흐름 전용 한국어 prompt를 반환한다."""

    return (
        "아래 기사 원문만 근거로 지난 월요일부터 일요일까지의 주간 뉴스 흐름을 "
        "작성하세요. 단일 사건 설명에 머물지 말고 한 주 동안의 변화, 진행 상황, "
        "반복해서 등장한 쟁점, 여러 출처가 공통으로 확인한 내용과 불확실한 "
        "내용을 구분하세요. 근거에 없는 사실은 추가하지 마세요. "
...
def _summary_evidence_articles_with_fallback(topic, raw_texts) -> list[dict]:
    """원문 없는 Summary 후보를 같은 Topic의 다음 관련 기사 원문으로 대체한다."""
    ...
    for article in related_articles:
        if len(selected) >= MAX_SUMMARY_EVIDENCE_COUNT:
            break
        article_id = int(article["id"])
        if article_id in selected_ids:
            continue
        if not (raw_texts.get(article_id) or "").strip():
            continue
...
def _build_topic_record(
    ...
    *,
    ...
    return WeeklyTopicRecord(
        ...
        model=model,
        prompt_version=PROMPT_VERSION,
        summary_input_hash=build_weekly_summary_input_hash(summary_input),
        articles=records,
    )
...
def _summary_schema() -> dict:
    """Responses API strict output에 사용할 주간 Topic Summary JSON schema를 반환한다."""
