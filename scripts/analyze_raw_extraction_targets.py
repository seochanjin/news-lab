...
DEFAULT_MAX_TARGETS_PER_TOPIC = 1
RAW_EXTRACTION_STATES_QUERY = text("""
    select
        article_id,
        extraction_status,
        coalesce(length(raw_text), 0) > 0 as has_raw_text
    from raw_articles
...
def parse_args(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description="Read-only representative-based raw extraction target analysis.",
    )
...
def get_raw_extraction_states(connection, article_ids):
    if not article_ids:
        return {}
    rows = connection.execute(
        RAW_EXTRACTION_STATES_QUERY,
        {"article_ids": list(article_ids)},
    ).mappings()
    return {row["article_id"]: dict(row) for row in rows}


def analyze(rows, raw_states, args, provider=None):
    ...
    target_topics = select_raw_extraction_targets(
        topic_candidates,
        raw_states,
        max_targets_per_topic=args.max_targets_per_topic,
    )
    return {
        "analysis": {
            "dry_run": True,
            ...
            "multi_article_topic_count": sum(
                topic["article_count"] > 1 for topic in target_topics
            ),
            "representative_candidate_count": sum(
                topic["representative_candidate_count"] for topic in target_topics
            ),
            "extraction_target_count": sum(
                topic["extraction_target_count"] for topic in target_topics
            ),
            "similarity_threshold": args.similarity_threshold,
            "max_candidates_per_topic": args.max_candidates_per_topic,
            "max_targets_per_topic": args.max_targets_per_topic,
        ...
        "topic_candidates": target_topics,
    }
...
def main():
    ...
    result = analyze(rows, raw_states, args)
    if args.report_path:
        args.report_path.parent.mkdir(parents=True, exist_ok=True)
        args.report_path.write_text(
            render_raw_extraction_target_report(result),
            encoding="utf-8",
        )
